    ...
    public boolean movePiece(Move move, int diceValue, int playerTurn, boolean isAI) {
        ...
        if (piece.getInt() == 1) {
            ...
            if (end.row == 0 || end.row == 7) {
                ...
                // we would need to change too much in our implementation for the algorithm to
                // support promoting to other pieces
        ...
        resetEnPassant();
        // if the pawn has moved two spaces make the adjacent enemy pawns be able to use
        // en passant
        ...
        if (piece.getInt() == 6) {
            ...
            if (startPos != null) {
                ...
                moveHistory.push(new MoveHistoryData(new Move(startPos, endPos), capturedPos, rook, null, true, null,
                        null, move50rule));
                rook.setPos(endPos);
            }
        ...
        if (piece.getInt() != 1 && captured == null) {
            move50rule++;// if no pawn has been moved and if no piece has been captured we increase the
                         // move50rule variable
        } else {
